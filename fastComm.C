...
void FastComm::config_tfc(uint8_t length, uint8_t command[], uint8_t period, bool singleShot) {
  fpga_->write_fpga(registers::TFC_CFG,(uint8_t) 0x01);   // reset TFC state machine
  usleep(1);
  fpga_->write_fpga(registers::TFC_CFG,(uint8_t) 0x00);
  fpga_->write_fpga(registers::TFC_LENGTH, length);   // Specify Length
  fpga_->write_fpga(registers::TFC_PERIOD0, period);	  // Specify period
  if(singleShot)  // Specify single shot or cont.
    fpga_->write_fpga(registers::TFC_CFG, (uint8_t)0x02); // single shot
  else
    fpga_->write_fpga(registers::TFC_CFG, (uint8_t)0x00); // continuous
  //cout << "about to specify list" << endl;
  for(uint i = 0; i < length; i++){   // specify command list
    fpga_->write_fpga(registers::TFC_WR, command[i]);
   // cout << "command[" << dec << i << "] = " << hex << (unsigned) command[i] << endl;
  }
}
...
void FastComm::read_daq(uint8_t length, uint32_t (&packet)[5120], bool tfc_trig) {
  uint32_t data;
  trigger_DAQ(tfc_trig);   																		// trigger aquisition if not tfc triggered
  fpga_->write_fpga(registers::DAQ_TRIGGER, (uint8_t)0x00 );  // stop all triggers
  for(uint i = 0; i < length; i++) { 													// read DAQ registers
    fpga_->read_fpga(registers::DAQ_READ0, &data);
    //data = arrange_Elinks(data);
    packet[i]=data;
...
//  while( ((data & 0x10) != 0x10) && ((clock()-begin)<26E-6*CLOCKS_PER_SEC));
  while( ((data & 0x10) != 0x10) && ((clock()-begin)<26E-5*CLOCKS_PER_SEC));
  if((data & 0x10) == 0) {
...
void FastComm::read_NZS_packet(uint16_t length_read, unsigned (&twelveBits)[10240], int startBit, int (&ADC)[128], int &bxid, int &parity, int &mcm_v, int &mcm_ch, int &mem_space) {
  ...
  // read DSP monitoring bits (CAT the two strings)
  dsp_mon = twelveBits[startBit+1] << 12;     //shift first 12b to the high part
  dsp_mon = dsp_mon | twelveBits[startBit+2]; //add the last 12b to the low part

  //cout << "DSP Mon = " << hex << (unsigned) dsp_mon << endl;
  
  mcm_v = (dsp_mon &     0xFC0000) >> 18;
  mcm_ch = (dsp_mon &    0x03FC00) >> 10;
  mem_space = (dsp_mon & 0x0003FE) >> 1;
  
  //cout << "MCM_CH = " << dec << (unsigned) mcm_ch << endl;
  
  for(int i=startBit+3; i < length_read*2; i++) {

    // read twelve bits at a time
    //twelveBits = read_twelveBits(data, i);
    //cout << hex << (unsigned) twelveBits[i] << endl;
