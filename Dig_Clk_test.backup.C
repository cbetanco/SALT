...
bool Dig_Clk_test::DAQ_Sync() {
  ...
  int e[8][8] = {0};
  int s[8][8] = {0}; //One sample from each...
  int bs_p[2];
  ...
  // Set correct pattern
  salt_->write_salt(registers::pattern_cfg, (uint8_t) 0x0AB); // Set pattern for synch, in this case hAB
  ...
  cout << i << ": ";
	...
	e[i][j]+=Check_Ber(data,length,0x01);   
/*	if(k==99 && (e[i][j]==0)) { //No shortcuts. We will measure it all
	  bs_p[0] = i;
	  bs_p[1] = j;
	  found_opt = true;
	  break;
...
//      if(found_opt) break;
 ...
 //With the definitive table we chose the first one that works
  ...
  if(!found_opt){  //Leave the bit slip where it initially was...
     ...
     for(int i = 0; i < 8; i++) {
       cout << i << ": ";
       for(int j = 0; j < 8; j++) cout << setfill('0') << setw(6) << hex << (0xFFFFFF & s[i][j]) << " ";
       cout << endl;
     }
...
bool Dig_Clk_test::DLL_Check_v3() {
  ...
  for (vcdl_value=0x127; vcdl_value > 0; vcdl_value--)  { //Only 16 phases to check!
      salt_->write_salt(registers::dll_vcdl_cfg, vcdl_value);
      usleep(1);
      salt_->read_salt(registers::dll_vcdl_mon, &read);
      //cout << "vcdl_value=" << hex << (int)vcdl_value << " dll_cur_OK="<< (bool)(read & 0x80) << endl;
...
bool Dig_Clk_test::TFC_Command_Check() {
  ...
  for(int i=0; i < 8; i++) {
    ...
    else if(option[i] == "NZS")      command[79] = 0x02, command[80] = 0x08;
